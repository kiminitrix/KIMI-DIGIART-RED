    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className={`p-6 rounded-3xl border ${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-red-100 shadow-sm'}`}>
          ...
          <label className={`w-full aspect-square border-2 border-dashed rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden
        ...
        {/* Result */}
        <div className={`p-6 rounded-3xl aspect-square border flex flex-col ${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-red-100 shadow-sm'}`}>
          <h3 className="font-bold text-lg mb-4">Edit Result</h3>
          {isEditing ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-4">
              <Loader2 className="animate-spin text-red-600" size={48} />
              <p className="animate-pulse font-medium">Refining pixels...</p>
            </div>
          ) : result ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center opacity-30">
              <Sparkles size={64} />
              <p className="mt-4 font-medium">Edit preview will appear here</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={`p-6 rounded-3xl border ${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-red-100 shadow-lg'}`}>
        <div className="flex items-end gap-4">
          <div className="flex-1">
            <label className="text-xs font-bold uppercase tracking-wider mb-1 block opacity-50">Modification Prompt</label>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. 'Add a cyberpunk visor', 'Change the sky to sunset'..."
              className={`w-full p-4 rounded-2xl min-h-[100px] focus:ring-2 focus:ring-red-600 outline-none transition-all resize-none
          ...
          <button
            ...
            className="flex-shrink-0 h-[100px] px-8 bg-red-600 hover:bg-red-700 text-white rounded-2xl font-bold flex flex-col items-center justify-center gap-2 shadow-xl shadow-red-900/20 transition-all disabled:opacity-50"
          >
